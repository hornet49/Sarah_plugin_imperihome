...
var config = config.modules.veralite;
  ...
  console.log("Missing IP_Imperihome config");
  return;
  }; 
  console.log(config.IP_Vera);
  console.log(config.IP_Imperihome);
...
var sendURL = function(url, callback){
  ...
  // Callback with random TTS
	random = Math.floor((Math.random()*(phrase_success.length-1))+1);
			...
			console.log("phrase_select= " + phrase_select);
			
  // Callback with Imperihome avec donnée du fichier .prop (qui ne marche pas)
  //    var ip_imp = config.IP_Imperihome;
  //    var url_imperihome = 'http://'+ip_imp+':8080/api/rest/speech/tts?text='+phrase_select;

  // Callback with Imperihome (qui fonctionne)
      var url_imperihome = 'http://192.168.0.73:8080/api/rest/speech/tts?text='+phrase_select
